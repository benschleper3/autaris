-- Creator vs UGC Creator Business OS Database Schema
...
-- RLS policies for Link hub tables
...
-- RLS policies for Portfolio and Reports
...
SELECT 
  ...
  ROUND(AVG(pm.engagement_rate), 2) as avg_engagement_rate,
...
SELECT 
  ...
  ROUND(AVG(pm.engagement_rate), 2) as avg_engagement_percent
FROM public.post_metrics pm
WHERE pm.published_at IS NOT NULL
GROUP BY pm.user_id, pm.platform, EXTRACT(DOW FROM pm.published_at), EXTRACT(HOUR FROM pm.published_at);

CREATE OR REPLACE VIEW public.v_pricing_suggestions AS
SELECT 
  pm.user_id,
  pm.platform,
  ROUND(AVG(pm.views)) as avg_views_30d,
  CASE 
    WHEN pm.platform = 'instagram' THEN 8.0
    WHEN pm.platform = 'tiktok' THEN 6.0
    WHEN pm.platform = 'youtube' THEN 12.0
    ELSE 7.0
  END as suggested_cpm_usd
FROM public.post_metrics pm
WHERE pm.created_at >= now() - interval '30 days'
GROUP BY pm.user_id, pm.platform;
